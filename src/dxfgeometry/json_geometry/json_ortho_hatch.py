...
class JsonOrthoHatch(JsonCADElement):
    points: list[float]
    pattern: str

    def __init__(self, hatch: OrthoHatch):
        super(JsonOrthoHatch, self).__init__(hatch.get_layer_name(), "OrthoHATCH")
        self.points = hatch.get_coord_list_flatten()
        self.pattern = hatch.get_hatch_pattern_name()


def json_ortho_hatch_to_ortho_hatch(json_hatch: dict, existing_angles: list[float]) -> OrthoHatch:
    ...
    return OrthoHatch(point_list, json_hatch["layer_name"], json_hatch["pattern"], existing_angles)
